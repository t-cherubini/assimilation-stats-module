...
def main(argc, argv):
  ...
  # Experiments labels
   ...
   for s in [0,1]:
      ...
      EXPDIR.append('/glade/scratch/cherubin/oper/post/validation/%s/ec_statdata/ts/' % exp[s])
   ...
   rootfigname_1 = ('profiles_wrfVSec_rms_t%02d+%02dh_%s_%d' % (cyc, fcsh, field, int(L) ))
   print(rootfigname_1)

   fig2, axs2 = plt.subplots(nrows=7, ncols=5, figsize=(30, 40))
   rootfigname_2 = ('ts_profiles_wrfVSec_rms_t%02d+%02dh_%s_%d' % (cyc, fcsh, field, int(L) ))
   print(rootfigname_2)
...
#   axs.set_title('Corrected RMS - WRF vs GFS analysis')
   ...
   for s in [0,1]:
      DATADIR = EXPDIR[s]
      analysis_file = ('%s/ec_ts.t%02dz+%02dh.nc' % (BASELINEDIR,cyc,fcsh))
