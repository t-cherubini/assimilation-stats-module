...
def main(argc, argv):
   ...
   for s in [0,1]:
      ...
      DATADIR.append('/glade/scratch/cherubin/oper/post/validation/%s/statdata/ts/' % exp[s])
   ...
   rootfigname = ('ts_wrfVSgfs_rms_%02dh_%s_%d' % (fcsh, field, int(L) ))
   print(rootfigname)
...
#                                 "ts_rmse_corr_m-oper","ts_rmse_corr_m-control","ts_rmse_corr_m-conv"]
   ...
   for s in [0,1]:
      ...
      for cyc in cycles:
         ...
         if(field == 'rh'):
             ts_rmse_corr_exp.plot.line(color=col,marker='o', ylim=[4,17])
             ts_rmse_corr_ref.plot.line(color='red',marker='o',ylim=[4,17])
