...
def main(argc, argv):
   ...
   if not (argc == 8):
        print ('Usage: %s <exp_name=ALL> <cycling hour> <fcst hour> <var to plot (rmse)> <level> <YYYY-MM-DD (start)> <YYYY-MM-DD (end)>' % argv[0])
        sys.exit(1)
  ...
  # Experiments labels
   ...
   rootfigname_1 = ('sub_profiles_wrfVSgfs_rms_t%02d+%02dh_%s_%d' % (cyc, fcsh, field, int(L) ))
   print(rootfigname_1)

   fig2, axs2 = plt.subplots(nrows=7, ncols=5, figsize=(30, 60))
   rootfigname_2 = ('sub_ts_profiles_wrfVSgfs_rms_t%02d+%02dh_%s_%d' % (cyc, fcsh, field, int(L) ))
   print(rootfigname_2)

   fig3, axs3 = plt.subplots(nrows=7, ncols=5, figsize=(15, 60))
   rootfigname_3 = ('err_sub_ts_profiles_wrfVSgfs_rms_t%02d+%02dh_%s_%d' % (cyc, fcsh, field, int(L) ))
   print(rootfigname_3)
...
#   axs.set_title('Corrected RMS - WRF vs GFS analysis')
   ...
   for s in [0,1]:
      ...
      rmse_exp_corr = np.sqrt(rmse_exp*rmse_exp - bias_exp*bias_exp) 
      rmse_ref_corr = np.sqrt(rmse_ref*rmse_ref - bias_ref*bias_ref) 
      ...
      ts_rmse_corr_exp = np.sqrt(ts_rmse_exp*ts_rmse_exp - ts_bias_exp*ts_bias_exp)
      ts_rmse_corr_ref = np.sqrt(ts_rmse_ref*ts_rmse_ref - ts_bias_ref*ts_bias_ref)
      ...
      vert_rmse_corr_exp = np.sqrt(vert_rmse_exp*vert_rmse_exp - vert_bias_exp*vert_bias_exp)
      vert_rmse_corr_ref = np.sqrt(vert_rmse_ref*vert_rmse_ref - vert_bias_ref*vert_bias_ref)

      D_err=(vert_rmse_corr_exp - vert_rmse_corr_ref)/vert_rmse_corr_ref*100.
      ...
      ts_vert_rmse_corr_exp = np.sqrt(ts_vert_rmse_exp*ts_vert_rmse_exp - ts_vert_bias_exp*ts_vert_bias_exp)
      ts_vert_rmse_corr_ref = np.sqrt(ts_vert_rmse_ref*ts_vert_rmse_ref - ts_vert_bias_ref*ts_vert_bias_ref)
   ...
   #   df_a = ds_a.to_dataframe()
      ...
      ax_right = axs1.twinx()
      D_err.plot.line(y='lev', ax=ax_right, xlim=[0,25], yincrease=False,color=err_col)
   ...
   #   vert_rmse_ref.plot.line(y='lev', yincrease=False, color='red', linestyle='dashed')
      ...
      for axis,t in zip(axs2.flat,range(len(times))):  
        if(field == 'rh'):
#          ts_D_err[t,:] = (ts_vert_rmse_corr_exp[t,:] - ts_vert_rmse_corr_ref[t,:])/ts_vert_rmse_corr_ref[t,:]

          ts_vert_rmse_corr_exp[t,:].plot.line(y='lev', ax=axis, xlim=[0,25], yincrease=False,color=col)
...
#        axis.xaxis.label.set_size(20)
      ...
      # -- Third Figure
      plt.figure(3) # Open figure for Time Serie of Vertical Profile of rMS

      # should plot vertical corrected rms for each day of timeserie

      # Calculate Error reduction 
      ts_D_err=100*(ts_vert_rmse_corr_exp - ts_vert_rmse_corr_ref)/ts_vert_rmse_corr_ref
      for axis,t in zip(axs3.flat,range(len(times))):
       #   ax_right = axis.twinx()
          ts_D_err[t,:].plot.line(y='lev', ax=axis, xlim=[-20,10], yincrease=False,color=err_col)
          axis.axvline(x=0, color='black', linestyle='dotted')
...
#        axis.xaxis.label.set_size(20)
  ...
  # Save Figures
   ...
   plt.figure(3)
   figname = ('%s_2.png' % rootfigname_3)
   plt.savefig(figname)
   figname = ('%s_2.eps' % rootfigname_3)
   plt.savefig(figname, format='eps')
