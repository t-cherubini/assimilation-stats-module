...
def main(argc, argv):
   ...
   if not (argc == 8):
        print ('Usage: %s <exp_name> <cycling hour> <fcst hour> <var to plot (rmse)> <level> <YYYY-MM-DD (start)> <YYYY-MM-DD (end)>' % argv[0])
        sys.exit(1)
...
#   print(exp_file)
   ...
   rootfigname = ('tsel_wrfVSgfs_rms_t%02d+%02dh_%s_%d' % (cyc, fcsh, field, int(L) ))
   print(rootfigname)
